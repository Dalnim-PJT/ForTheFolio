...
def m_create(request):
    if request.method == "POST":
        ...
        pjt = [
            PjtForm(request.POST, prefix=f"pjt-{i}")
            for i in range(
                int([k for k in request.POST if k.startswith("pjt-")][-1][4]) + 1
            )
        ]
        career = [
            CareerForm(request.POST, prefix=f"career-{i}")
            for i in range(
                int([k for k in request.POST if k.startswith("career-")][-1][7]) + 1
            )
        ]
        link = [
            LinkForm(request.POST, prefix=f"link-{i}")
            for i in range(
                int([k for k in request.POST if k.startswith("link-")][-1][5]) + 1
            )
        ]
        pjtimage = [
            PjtImageForm(request.POST, request.FILES, prefix=f"pjt-{i}")
            for i in range(
                int([k for k in request.POST if k.startswith("pjt-")][-1][4]) + 1
            )
        ]
                ...
                if p_form.is_valid():
                    ...
                    if name:
                        ...
                        if pjtimage[i].is_valid():
                            ...
                            if images:
                                for image in images:
                                    Pjtimages.objects.create(
                                        image=image, mydata=my_data, pjt=my_pjt
                                    )
...
def m_update(request, mydata_title):
    ...
    if request.method == "POST":
        ...
        update_pjt_ids = [
            ...
            if k.startswith("pjt-") and k.endswith("name")
        ]
        # Delete pjts
        ...
        if basic.is_valid():
            ...
            for i, pjt_id in enumerate(update_pjt_ids):
                try:
                    ...
                    pjt_instance = None
                pjt_form = PjtForm(
                    request.POST, prefix=f"pjt-{pjt_id}", instance=pjt_instance
                )
                pjt_image_form = PjtImageForm(
                    request.POST, request.FILES, prefix=f"pjt-{pjt_id}"
                )
                pjt_stack_form = (
                    [stack.stack for stack in pjt_instance.stack.all()]
                    if pjt_instance
                    else []
                )
                ...
                if pjt_form.is_valid():
                    ...
                    for my_pjt_stack in pjt_stack_form:
                        if my_pjt_stack not in p_stacks:
                            pjt_instance.stack.remove(
                                TechStack.objects.get(stack=my_pjt_stack)
                            )

                    # Add images
                    if pjt_image_form.is_valid():
                        images = request.FILES.getlist(
                            f"pjt-{pjt_instance.id}-image"
                        ) or request.FILES.getlist(f"pjt-0{i}-image")
                        for image in images:
                            Pjtimages.objects.create(
                                image=image, mydata=my_data, pjt=pjt_instance
                            )

                    # Delete images
                    pjt_image_delete_form = DeletePjtImageForm(
                        prefix=f"delete-pjt-{pjt_id}",
                        pjt=pjt_instance,
                        data=request.POST,
                    )
                    if pjt_instance and pjt_image_delete_form.is_valid():
                        pjt_instance.pjtimages_set.filter(
                            pk__in=pjt_image_delete_form.cleaned_data["delete_images"]
                        ).delete()
            ...
            update_career_ids = [
                k.split("-")[1]
                for k in request.POST
                if k.startswith("career-") and k.endswith("career_content")
            ]
            for career_id in update_career_ids:
                c_form = CareerForm(
                    request.POST,
                    prefix=f"career-{career_id}",
                    instance=Career.objects.filter(pk=career_id).first(),
                )
                if c_form.is_valid():
                    name = c_form.cleaned_data["career_content"]
                    if name:
            ...
            update_link_ids = [
                k.split("-")[1]
                for k in request.POST
                if k.startswith("link-") and k.endswith("link_content")
            ]
            for link_id in update_link_ids:
                l_form = LinkForm(
                    request.POST,
                    prefix=f"link-{link_id}",
                    instance=Links.objects.filter(pk=link_id).first(),
                )
                if l_form.is_valid():
        ...
        pjts = [
            ...
            for pjt in my_pjts
        ] or [[PjtForm(prefix="pjt-0"), PjtImageForm(prefix="pjt-0")]]
        careers = [
            CareerForm(prefix=f"career-{str(career.id)}", instance=career)
            for career in my_careers
        ] or [CareerForm(prefix="career-0")]
        links = [
            LinkForm(prefix=f"link-{str(link.id)}", instance=link) for link in my_links
        ] or [LinkForm(prefix="link-0")]

    return render(
        request,
        "portfolios/m_update.html",
        {
            "stacks": TechStack.STACK_CHOICES,
            "my_data": my_data,
            "basic": basic,
...
def p_create(request, template_name):
    ...
    if selected_data == "포트폴리오 예시":
        selected_mydata = Mydatas.objects.get(
            user_id=User.objects.get(username="admin")
        )
    else:
        selected_mydata = Mydatas.objects.get(
            user_id=request.user.id, title=selected_data
        )
    ...
    if request.method == "POST":
            ...
            for i, pjt_id in enumerate(update_pjt_ids):
                ...
                if pjt_form.is_valid():
                    ...
                    if pjt_image_form.is_valid():
                        images = request.FILES.getlist(
                            f"pjt-{pjt_instance.id}-image"
                        ) or request.FILES.getlist(f"pjt-{i}-image")
                        for image in images:
                            Pjtimages.objects.create(
                                image=image, portfolio=portfolio_data, pjt=pjt_data
                            )
                        ...
                        if pjt_instance and pjt_image_delete_form.is_valid():
                            for image in pjt_instance.pjtimages_set.exclude(
                                pk__in=pjt_image_delete_form.cleaned_data[
                                    "delete_images"
                                ]
                            ):
                                Pjtimages.objects.create(
                ...
                if c_form.is_valid():
                    ...
                    if c_content:
                        Career.objects.create(
                            career_content=c_content, portfolio=portfolio_data
                        )
                ...
                if l_form.is_valid():
                    ...
                    if l_content:
                        Links.objects.create(
                            link=l_form.cleaned_data["link"],
                            link_content=l_content,
                            portfolio=portfolio_data,
                        )
        ...
        careersform = [
            CareerForm(prefix=f"career-{str(career.id)}", instance=career)
            for career in selected_my_careers
        ] or [CareerForm(prefix="career-0")]
        linksform = [
            LinkForm(prefix=f"link-{str(link.id)}", instance=link)
            for link in selected_my_links
        ] or [LinkForm(prefix="link-0")]

    return render(
        request,
        f"portfolios/temp_portfolio/{template_name}.html",
        {
        ...
        if portfolio.is_valid():
            ...
            for my_portfolio_stack in my_portfolio_stacks:
                if my_portfolio_stack not in b_stacks:
                    my_portfolio.stack.remove(
                        TechStack.objects.get(stack=my_portfolio_stack)
                    )
            ...
            for i, pjt_id in enumerate(update_pjt_ids):
                ...
                if pjt_form.is_valid():
                    ...
                    if pjt_image_form.is_valid():
                        ...
                        for image in images:
                            Pjtimages.objects.create(
                                image=image, portfolio=my_portfolio, pjt=pjt_instance
                            )
